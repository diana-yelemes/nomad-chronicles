...
const Figures = () => {
  ...
  const applyFilters = () => {
    let filtered = [...figures]; // Updated to use figures
    ...
    if (category.length) {
      filtered = filtered.filter(figure => category.includes(figure.category));
    }
    return filtered;
  };
  ...
  const applySorting = (figuresList) => {
    switch (sortType) {
      case 'oldest':
        return figuresList.sort((a, b) => b.birthYear - a.birthYear);
      case 'newest':
        return figuresList.sort((a, b) => a.birthYear - b.birthYear);
      default:
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
  }, [category, sortType, figures, search]); // Updated dependency
    ...
    <section className="max-padd-container bg-white">
      <div className="pt-28">
        ...
        <div className="mt-12 mb-16">
          ...
          <div className="flex-center sm:flexStart flex-wrap gap-x-12 gap-y-4">
              ...
              <label key={category.name}>
                ...
                <div className="flexCenter flex-col gap-2 peer-checked:text-secondaryOne cursor-pointer">
                  <div className="bg-primary h-20 w-20 flexCenter rounded-full">
                    <img src={category.image} alt={category.name} className="object-cover h-10 w-10" />
                  </div>
        ...
        <div className="mt-8">
          <div className="flexBetween !items-start gap-7 flex-wrap pb-16 max-sm:flexCenter text-center">
            <Title title1={'Our'} title2={' Figures List'} titleStyles={'pb-0 text-start'} paraStyles={'!block'} />
